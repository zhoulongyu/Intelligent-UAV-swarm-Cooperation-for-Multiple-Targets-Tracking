...
def get_agents_action(o_n, sess, noise_rate=0):
    agent1_action = agent1_ddpg.action(state=[o_n[0]], sess=sess) + np.random.randn(4) * noise_rate
    agent2_action = agent2_ddpg.action(state=[o_n[1]], sess=sess) + np.random.randn(4) * noise_rate
    agent3_action = agent3_ddpg.action(state=[o_n[2]], sess=sess) + np.random.randn(4) * noise_rate
    agent4_action = agent4_ddpg.action(state=[o_n[3]], sess=sess) + np.random.randn(4) * noise_rate
...
if __name__ == '__main__':
    ...
    for i in range(10000):
        ...
        agent1_action, agent2_action, agent3_action, agent4_action = get_agents_action(o_n, sess)
